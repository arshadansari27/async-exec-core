...
class AsyncExecutor(object):
    ...
    def publisher(self, out_channel, out_queue):
        def decorator(func):
            logger.info("Registering handler {} for channel: {} on queues ({})".format(
            ...
            logger.info('flow [publisher: {}] {} {} {}'.format(flow.id, out_channel, out_queue, 'ready'))
    ...
    def listener(self, in_channel, in_queue, max_workers=4):
        def decorator(func):
            ...
            flow = Flow(config, loop=self.loop)\
                ...
                .add_sink(func)
            self.flows.append(flow)
            logger.info('flow [subscriber: {}] {} {} {}'.format(flow.id, in_channel, in_queue, 'ready'))
    ...
    def handler(self, channel, queue_request, queue_response, max_workers=4):
        def decorator(func):
            ...
            self.flows.append(flow)
            logger.info('flow [handler: {}] {} {} {} {}'.format(flow.id, channel, queue_request, queue_response, 'ready'))
    ...
    def handle_and_collect(self, in_channel, in_queue, reducer=None, callback=None, count=None, max_workers=4):
        def decorator(func):
            logger.info("Registering collector {} for channel: {} on queue ({}) with callback {}".format(
            ...
            self.flows.append(flow)
            logger.info('flow [handle and collect: {}] {} {} {}'.format(flow.id, in_channel, in_channel, 'ready'))

            return func
        return decorator

    def handle_and_publish_many(self, in_channel, in_queue, out_channel, out_queue, max_workers=4):
        def decorator(func):
            logger.info("Registering handler and broadcast '{}' for channel: {} and {} and on queue ({}, {}) ".format(
                func.__name__, in_channel, out_channel, in_queue, out_queue
            ))
            assert in_channel in self.channel_configurations
            host, port, username, password = self.channel_configurations[in_channel]
            config = {
            ...
            self.flows.append(flow)
            logger.info('flow [handler and publish many: {}] {} {} {} {} {}'.format(flow.id, in_channel, in_queue, out_channel, out_queue, 'ready'))
    ...
    def handle_and_broadcast(self, in_channel, in_queue, out_channel, *out_queues, max_workers=4):
        def decorator(func):
            ...
            self.flows.append(flow)
            logger.info('flow [handler and broadcast: {}] {} {} {} {} {}'.format(flow.id, in_channel, in_queue, out_channel, ','.join(out_queues), 'ready'))
    ...
    def collector(self, in_channel, in_queue, callback=None, count=None, max_workers=4):
        def decorator(func):
            ...
            self.flows.append(flow)
            logger.info('flow [collector: {}] {} {} {}'.format(flow.id, in_channel, in_channel, 'ready'))
